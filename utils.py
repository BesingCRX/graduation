...
def sample_gt(gt, train_size, LABEL_VALUES, mode='random'):
    ...
    if mode == 'random':
    ...
    elif mode == 'ten_fold_cross_validation':
        ten_fold_splits = {'train': [], 'test': []}
        kf = KFold(n_splits=10, shuffle=True)
        label_counts = Counter(gt.flatten())
        ...
        for train_index, test_index in kf.split(gt):
            ...
            for label, count in label_counts.items():
                ...
                sampled_indices = np.random.choice(indices, int(count * train_size), replace=True)

                train_label_indices = sampled_indices[np.isin(sampled_indices, train_index)]
                test_label_indices = sampled_indices[np.isin(sampled_indices, test_index)]
            ...
            ten_fold_splits['train'].append(train_gt)
            ten_fold_splits['test'].append(test_gt)

        return ten_fold_splits
