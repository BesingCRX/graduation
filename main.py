...
from models import get_model, train, test, save_model, train_with_cross_validation, MyModel_Chen
...
for run in range(N_RUNS):
    if TRAIN_GT is not None and TEST_GT is not None:
        ...
        train_gt, test_gt = sample_gt(gt, SAMPLE_PERCENTAGE, LABEL_VALUES, mode=SAMPLING_MODE)
    ...
    if MODEL == 'SVM_grid':
        ...
        try:
            ...
            if dataset == 'Botswana' or dataset == 'KSC' or dataset == 'IndianPines':
                # 使用十折交叉验证进行训练
                ten_fold_splits = sample_gt(gt, 0.9, LABEL_VALUES, mode='ten_fold_cross_validation')

                # 对每次划分进行训练
                for i, (train_gt, val_gt) in enumerate(zip(ten_fold_splits['train'], ten_fold_splits['test']), 1):
                    ...
                    # 在这里进行模型训练，验证等操作
                    train(model, optimizer, loss, train_loader, hyperparams, hyperparams['epoch'], dataset_name=hyperparams['dataset'],
                ...
                """                if dataset == 'IndianPines':
                                        ...
                                        train_gt, val_gt = sample_gt(train_gt, 0.9, LABEL_VALUES, mode='random')
                                    else:"""
                #分割训练数据
                train_gt, val_gt = sample_gt(train_gt, 0.3, LABEL_VALUES, mode='random')
                ...
                i=0
                while i<10:
                    train(model, optimizer, loss, train_loader, hyperparams, hyperparams['epoch'],
